...
def retirar_estoque(produtos)
  ...
  unless produto
    ...
    retirar_estoque(produtos) if %w[s sim].include?(opcao)

    return
  end

  limpar_tela
  mensagem_azul("Digite a quantidade que deseja retirar do estoque do produto: #{amarelo(produto[:nome])}", false,
                false)
